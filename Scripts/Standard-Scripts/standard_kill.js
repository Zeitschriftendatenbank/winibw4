...
var gPicaKill = 
{
	selectCorrectPPN : function() {
		...
		application.activeWindow.appendMessage(msg, 3);
	},
	...
	correctLinks : function() {	
		...
		for (var i = 1; i <= count; i++) {
			...
			if (gPicaUtility.isAuthority(application.activeWindow.materialCode)) {
				...
				iRecords++;
				application.activeWindow.title.replaceAll(this.__incorrectPPN, this.__correctPPN, false, true);
//				gConfig.insertRequiredTags();
		...
		application.activeWindow.showMessage(msg, 3);
	...
	simpleMessage : function(strId) {
		application.messageBox("WinIBW 4", gPicaUtility.getMessage(strId), "error-icon");
	},
