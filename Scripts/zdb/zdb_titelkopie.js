...
function zdb_Parallelausgabe(){
    if(false == __zdbCheckScreen(['8A'],'Parallelausgabe')) {
        return false;
    }
    var matCode = activeWindow.materialCode.charAt(0);
    if(matCode == 'T' || matCode == 'O') {
...
function __zdbOnlineRessource(copyFile, showComment, add0600, digi) {
    ...
    if(zdb._rec['017A'])
    {
        var _codes = __zdbArrayDiff(zdb._rec['017A'][0]['a'], ['es', 'ks', 'sf', 'sm', 'mg', 'mm', 'nw', 'ra', 'rb', 'rc', 'rg', 'ru', 'ee', 'vt']);
        // join arrays
    ...
    for(var f in _felder424X) //  f = 039.
    {
        ...
        if(zdb._rec[f]) // Feld 039. vorhanden
        {
            for(var e in zdb._rec[f]) // Wiederholungen
            {
                ...
                code = (__zdbCheckSF(f,'b',e)) ? _code[zdb._rec[f][e]['b'][0]] : '';
                ...
                {
                    _exp = __zdbParseExpansion(zdb._rec[f][e][8][0]);
                ...
                {
                    text = code;
                    if(__zdbCheckSF(f,'n',e)) text += '$n'+zdb._rec[f][e]['n'][0];
                    if(__zdbCheckSF(f,'l',e)) text += '$l'+zdb._rec[f][e]['l'][0];
