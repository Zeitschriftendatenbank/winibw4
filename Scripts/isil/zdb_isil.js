...
function isil_OSM() {
    ...
    var name = application.activeWindow.title.findTag2("110", 0, false, false, false),
        ...
        plz = feld371.match(/\$e([^$]*)/);
    var adr = name + " " + plz[1] + " " + city[1];


    var xmlrow, display_name, lat, lon, format = '&format=xml';
    var ui = 'https://nominatim.openstreetmap.org/ui/search.html?';
    var base = "https://nominatim.openstreetmap.org/search.php?";
    var query = "street=" + encodeURIComponent(street[1]);
    query += "&city=" + encodeURIComponent(city[1]);
    query += "&postalcode=" + encodeURIComponent(plz[1]);
    query += "&country=" + encodeURIComponent(cntr[1]);
